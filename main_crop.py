...
import utils
from data_crop import WavDataset, WavTestDataset
    ...
    with torch.no_grad():
        ...
        with open('test_result.csv', 'w') as f:
            ...
            for o, n in zip(outputs, names):
                try:
                    final_outputs[n].append(int(o))
                except:
                    final_outputs[n] = [int(o)]

            for k, v in final_outputs.items():
                c = Counter(v)
                d = dict(c)
                preds = sorted(d.items(), key=lambda item: item[1], reverse=True)
                vote_pred = preds[0][0]
                f.write(f'{k},{label2name[vote_pred]}\n')



def save_checkpoint(state, is_best, epoch):
    global args
    utils.maybe_makedir('./weights-crop/')
    filename='./weights-crop/checkpoint_cry_{}-{:0>4}.pth'.format(args.arch, epoch)
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, './weights-crop/model_best_cry_{}.pth'.format(args.arch))
